/*
Package migrations supplies the subject-key table's DDL, rendered for a dialect
and table prefix.
...
repository, nothing to keep in sync as this package evolves:
	...
	m, err := migrate.New(dialect.Postgres, myMigrations,
		migrate.WithGeneratedMigration(43, "create_shredding_tables", ddl),
	)

Where you run it matters more here than it does for most schemas. This table
holds the only copy of every data subject's key, and a snapshot of it that
outlives a shred undoes the shred. Running it against the same database as the
data it protects is supported and is usually the wrong choice — see the
shredding package documentation.
