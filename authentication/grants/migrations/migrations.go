/*
Package migrations supplies the third-party grant table's DDL, rendered for a
...
repository, nothing to keep in sync as this package evolves:
	...
	ddl, err := migrations.SQL(dialect.Postgres, grants.DefaultTablePrefix)
	// ...
	m, err := migrate.New(dialect.Postgres, myMigrations,
		migrate.WithGeneratedMigration(44, "create_oauth2_grants_table", ddl),
	)
...
// It is what you hand to database/migrate's WithGeneratedMigration, so the grant
