/*
Package webauthnsessions stores WebAuthn ceremony state in a SQL table.
...
# Ceremonies, not credentials

What this stores is a ceremony in flight, and nothing else. A registered
credential — its ID, its public key, its sign count — is the application's to
keep, which is what primitives-go's authentication/webauthn says on the
SessionStore this implements: assembling a webauthn.User means knowing what a
user is, and only the directory holding them can answer that. This package is
named for the table it has rather than the one it does not.
...
internal/webauthnsessionsdb from the same files — typed params and methods over driver
