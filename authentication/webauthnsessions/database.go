...
const serviceName = "webauthnsessions_database"
...
func NewSessionStore(cfg *Config, db database.Client, opts ...Option) (*SessionStore, error) {
	...
	// substitution; see internal/webauthnsessionsdb.
	qd, err := webauthnsessionsdbDialect(d)
	if err != nil {
		return nil, err
	}
	...
	if s.q, err = webauthnsessionsdb.New(qd, ddl.Qualify(cfg.TablePrefix)); err != nil {
...
// webauthnsessionsdbDialect maps this module's dialect names onto the generated
...
func webauthnsessionsdbDialect(d dialect.Dialect) (webauthnsessionsdb.Dialect, error) {
	switch d {
	case dialect.Postgres:
		return webauthnsessionsdb.DialectPostgreSQL, nil
	case dialect.MySQL:
		return webauthnsessionsdb.DialectMySQL, nil
	case dialect.SQLite:
		return webauthnsessionsdb.DialectSQLite, nil
	default:
...
func (s *SessionStore) Save(ctx context.Context, session *webauthn.SessionData, ttl time.Duration) error {
	...
	if err = s.q.UpsertSession(ctx, s.db.Writer(), webauthnsessionsdb.UpsertSessionParams{
...
func (s *SessionStore) Consume(ctx context.Context, challenge string) (*webauthn.SessionData, error) {
	...
	var row webauthnsessionsdb.GetSessionRow
...
func (s *SessionStore) take(
	...
	q database.Tx,
	challenge string,
) (webauthnsessionsdb.GetSessionRow, error) {
	row, err := s.q.GetSession(ctx, q, webauthnsessionsdb.GetSessionParams{Challenge: challenge})
	if err != nil {
	...
	affected, err := s.q.DeleteSession(ctx, q, webauthnsessionsdb.DeleteSessionParams{Challenge: challenge})
	if err != nil {
