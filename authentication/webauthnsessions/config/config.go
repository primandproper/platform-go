...
type Config struct {
	...
	Database           webauthnsessions.Config `env:",init"    envPrefix:"DATABASE_" json:"database,omitzero" yaml:"database,omitempty"`
...
func newSessionStore(
	...
	case ProviderDatabase:
		...
		store, err := webauthnsessions.NewSessionStore(&cfg.Database, db, append([]webauthnsessions.Option{
			webauthnsessions.WithLogger(o.logger),
			webauthnsessions.WithTracerProvider(o.tracerProvider),
			webauthnsessions.WithMetricsProvider(o.metricsProvider),
			// Bound to the caller's context: the sweep stops when whatever
			// scope owns this store does.
			webauthnsessions.WithSweeper(ctx, pointer.Dereference(cfg.SweepInterval)),
		}, o.databaseStore...)...)
		if err != nil {
			return nil, err
		}
