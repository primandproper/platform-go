...
func (httpMapper) Map(err error) (code httperrors.ErrorCode, msg string, ok bool) {
	...
	// The three refusals on authority. All are 403: the caller is who they say
	// they are and this registration is not theirs to act on.
	case errors.Is(err, ErrOwnerMismatch):
		return httperrors.ErrUserIsNotAuthorized, "no such oauth2 client", true
...
func (grpcMapper) Map(err error) (code codes.Code, ok bool) {
	...
	// they are in all three cases, and it is the registration that is not
	// theirs. A client library treats the two differently, and answering
	// Unauthenticated would send a give-up path down the retry-with-credentials
	// branch.
	case errors.Is(err, ErrOwnerMismatch),
		errors.Is(err, ErrClientScopeMismatch), errors.Is(err, ErrClientOwnerMismatch):
