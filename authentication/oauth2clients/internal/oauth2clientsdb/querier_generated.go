// Code generated by github.com/primandproper/sqlc-gen-unison. DO NOT EDIT.
//
// sqlc:   v1.31.1
// unison: v0.3.0

package oauth2clientsdb

import (
	"context"
)

// Querier is every query in this package, with one signature per query
// regardless of which dialect answers it.
//
// That is the point of this package: the params type, the row type, and the
// method signature are the same on every dialect, so nothing above this line
// knows or cares which database is underneath.
//
// # A note on the :execrows count
//
// MySQL reports rows *changed*: an UPDATE that sets a column to the value it
// already held affects zero rows there. Postgres and SQLite report rows
// *matched*, and count that same UPDATE as one.
//
// So a statement that gates on the count — treating zero as "not found" — is
// correct on two engines and wrong on the third. Either give it a predicate
// that discriminates, or set clientFoundRows=true in the MySQL DSN, which
// switches MySQL to matched semantics.
type Querier interface {
	// ArchiveRegisteredClient runs the :execrows query.
	//
	// The count means different things on different engines; see the note
	// on Querier.
	ArchiveRegisteredClient(ctx context.Context, db DBTX, arg ArchiveRegisteredClientParams) (int64, error)
	// CreateRegisteredClient runs the :execrows query.
	//
	// The count means different things on different engines; see the note
	// on Querier.
	CreateRegisteredClient(ctx context.Context, db DBTX, arg CreateRegisteredClientParams) (int64, error)
	// GetRegisteredClient runs the :one query.
	GetRegisteredClient(ctx context.Context, db DBTX, arg GetRegisteredClientParams) (GetRegisteredClientRow, error)
	// GetRegisteredClientByClientID runs the :one query.
	GetRegisteredClientByClientID(ctx context.Context, db DBTX, arg GetRegisteredClientByClientIDParams) (GetRegisteredClientByClientIDRow, error)
	// GetRegisteredClientCreatedAt runs the :one query.
	GetRegisteredClientCreatedAt(ctx context.Context, db DBTX, arg GetRegisteredClientCreatedAtParams) (GetRegisteredClientCreatedAtRow, error)
	// ListRegisteredClients runs the :many query.
	ListRegisteredClients(ctx context.Context, db DBTX, arg ListRegisteredClientsParams) ([]ListRegisteredClientsRow, error)
	// ListRegisteredClientsDescending runs the :many query.
	ListRegisteredClientsDescending(ctx context.Context, db DBTX, arg ListRegisteredClientsDescendingParams) ([]ListRegisteredClientsDescendingRow, error)
	// ListRegisteredClientsForOwner runs the :many query.
	ListRegisteredClientsForOwner(ctx context.Context, db DBTX, arg ListRegisteredClientsForOwnerParams) ([]ListRegisteredClientsForOwnerRow, error)
	// ListRegisteredClientsForOwnerDescending runs the :many query.
	ListRegisteredClientsForOwnerDescending(ctx context.Context, db DBTX, arg ListRegisteredClientsForOwnerDescendingParams) ([]ListRegisteredClientsForOwnerDescendingRow, error)
	// UpdateRegisteredClient runs the :execrows query.
	//
	// The count means different things on different engines; see the note
	// on Querier.
	UpdateRegisteredClient(ctx context.Context, db DBTX, arg UpdateRegisteredClientParams) (int64, error)
}
