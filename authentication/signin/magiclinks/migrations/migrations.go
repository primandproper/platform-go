/*
Package migrations supplies the sign-in link table's DDL, rendered for
...
repository, nothing to keep in sync as this package evolves:
	...
	ddl, err := migrations.SQL(dialect.Postgres, magiclinks.DefaultTablePrefix)
	// ...
	m, err := migrate.New(dialect.Postgres, myMigrations,
		migrate.WithGeneratedMigration(46, "create_signin_magic_links_table", ddl),
	)
...
// table and the two indexes this package creates.
//
// The indexes are the reason this is not a check on the prefix alone: the
// longest identifier here is the sweeper's index, at 35 bytes before a prefix is
