/*
Package migrations supplies the recovery code table's DDL, rendered for
...
repository, nothing to keep in sync as this package evolves:
	...
	ddl, err := migrations.SQL(dialect.Postgres, recoverycodes.DefaultTablePrefix)
	// ...
	m, err := migrate.New(dialect.Postgres, myMigrations,
		migrate.WithGeneratedMigration(46, "create_signin_recovery_codes_table", ddl),
	)
...
// splits it into individually executable statements.
...
//
// It is vetted against the rendered name rather than as a string of its own,
// because a prefix that is a legal identifier alone can still push the table's
// name past what an engine accepts — a failure that would otherwise surface as a
