...
// DefaultSecretBytes is how much randomness a refresh token carries when no
...
//
// Thirty days, and the length is not what matters — being greater than zero is.
...
// whose row is gone reads as an unknown one, which is the answer that lets a
// thief's second presentation pass unnoticed. The window is how long after a
// family lapses that detection still works.
...
// therefore signs out everybody holding a refresh token. Choose it once.
