...
// The login most of these tests exercise.
const (
	testFamilyID = "family_01"
	testSubject  = "user_01"
	testAccount  = "account_01"
)
...
func newTestClient(tb testing.TB) database.Client {
	...
	client, err := sqlite.NewDatabaseClient(tb.Context(),
		&testClientConfig{connectionString: filepath.Join(tb.TempDir(), "refreshtokens.db")})
	must.NoError(tb, err)
...
// issue mints one token into the usual login, failing the test if it cannot.
func issue(tb testing.TB, store *SQLStore) *signin.RefreshTokenIssuance {
	tb.Helper()
...
// issueFor mints one token from a named request, reporting what the store said
...
//
// It commits on ErrRefreshTokenReused rather than returning it out of the
// callback, which is the obligation that sentinel carries and the shape
// signin.Service.ExchangeRefreshToken has: the family revocation a detected
// reuse triggers is written into the same transaction, so unwinding it would
// detect the theft and then erase the response to it. Every other refusal rolls
// back, because every other refusal wrote nothing.
func redeem(tb testing.TB, store *SQLStore, scope tenancy.Scope, secret string) (*signin.RefreshToken, error) {
	tb.Helper()
	...
	err := withTx(tb, store, func(tx database.Tx) error {
		...
		token, redeemErr = store.Redeem(tb.Context(), tx, scope, secret)
		if platformerrors.Is(redeemErr, signin.ErrRefreshTokenReused) {
			reuse = redeemErr
...
// revokeFamily ends one login in a transaction of its own.
func revokeFamily(tb testing.TB, store *SQLStore, scope tenancy.Scope, familyID string) (int64, error) {
	tb.Helper()
	...
	err := withTx(tb, store, func(tx database.Tx) error {
		var revokeErr error
		revoked, revokeErr = store.RevokeFamily(tb.Context(), tx, scope, familyID)
