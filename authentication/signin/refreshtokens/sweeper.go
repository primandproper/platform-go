...
const sweptKey = "signin.refresh_tokens_swept"
...
const backgroundSweepFailure = "background sweep of collectable refresh token rows failed"

// Sweep removes every row past its purge deadline, reporting how many it
// removed.
//
// It is not what makes a refresh token stop working — the exchange's own guard
...
// that never existed, and that difference is the whole of reuse detection. See
// WithRetention.
//
...
func (s *SQLStore) Sweep(ctx context.Context) (int64, error) {
	...
	swept, err := s.q.SweepRefreshTokens(ctx, s.db.Writer(),
		signindb.SweepRefreshTokensParams{PurgeBefore: s.clock.Now().UTC()})
	if err != nil {
		s.sweepErrorsCounter.Add(ctx, 1)

		return 0, op.Error(err, "sweeping collectable refresh token rows")
	}
...
func newSweepInstruments(provider metrics.Provider) (swept, errs metrics.Int64Counter, err error) {
	...
	if swept, err = mp.NewInt64Counter(fmt.Sprintf("%s_rows_swept", serviceName)); err != nil {
		return nil, nil, platformerrors.Wrap(err, "creating swept refresh token rows counter")
	}

	if errs, err = mp.NewInt64Counter(fmt.Sprintf("%s_sweep_errors", serviceName)); err != nil {
		return nil, nil, platformerrors.Wrap(err, "creating refresh token sweep errors counter")
	}
