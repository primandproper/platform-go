...
const heldNoun = "password reset tokens"
...
func (c *Collector) Collect(
	...
	// not paged. ListForUser returns the whole set, on the structural bound the
	// package documentation states — the sweeper deletes every row at its own
	// expiry — so there is no cursor for the paging helper to walk.
	err := dataprivacy.ForEachOwner(ctx, c.resolve, requestScope, subject,
		func(ctx context.Context, scope tenancy.Scope) error {
			...
			if collectErr != nil {
				return platformerrors.Wrapf(collectErr, "collecting password reset tokens in scope %q", scope)
			}
