...
import (
	...
	"github.com/primandproper/platform-go/v14/authentication/passwordreset/privacy"
...
// The two scopes a subject's reset tokens might be in. Two, because the whole of
...
// tokenIn is one stored token, in a scope.
func tokenIn(scope tenancy.Scope, id string) *passwordreset.Token {
	return &passwordreset.Token{
		ID:        id,
		Scope:     scope,
		UserID:    subject.ID,
		CreatedAt: time.Date(2026, time.September, 15, 12, 0, 0, 0, time.UTC),
		ExpiresAt: time.Date(2026, time.September, 15, 13, 0, 0, 0, time.UTC),
	}
}
...
func TestRequestScope(T *testing.T) {
	...
	T.Run("refuses a request that names none", func(t *testing.T) {
		...
		// every reset the subject ever asked for.
...
func TestNewCollector(T *testing.T) {
	...
	T.Run("refuses what it cannot work without", func(t *testing.T) {
		...
		_, err = privacy.NewCollector(&passwordresetmock.StoreMock{}, nil, privacy.RequestScope)
		must.ErrorIs(t, err, privacy.ErrNilExecutor)

		_, err = privacy.NewCollector(&passwordresetmock.StoreMock{}, &testReader{}, nil)
...
func TestCollector_Collect(T *testing.T) {
	...
	T.Run("collects every scope the resolver names", func(t *testing.T) {
		...
		store := &passwordresetmock.StoreMock{
			ListForUserFunc: func(
				...
				return []*passwordreset.Token{tokenIn(scope, "token_in_"+scope.String())}, nil
			},
		}
		...
		var collected []passwordreset.Token
		must.NoError(t, json.Unmarshal(fragment, &collected))
		must.SliceLen(t, 2, collected)
		test.EqOp(t, "token_in_acct_1", collected[0].ID)
		test.EqOp(t, "token_in_acct_2", collected[1].ID)

		// What a subject is entitled to: that they asked, when, and until when.
		test.False(t, collected[0].CreatedAt.IsZero())
		test.False(t, collected[0].ExpiresAt.IsZero())
	})

	T.Run("carries nothing that could be exchanged for a link", func(t *testing.T) {
		t.Parallel()

		// The store's projection excludes token_digest, so there is no field on
		// Token for a secret or a digest to arrive in. This asserts it against
		// the bytes the artifact would carry.
		store := &passwordresetmock.StoreMock{
			ListForUserFunc: func(
				_ context.Context, _ database.SQLQueryExecutor, scope tenancy.Scope, _ string,
			) ([]*passwordreset.Token, error) {
				return []*passwordreset.Token{tokenIn(scope, "token_1")}, nil
			},
		}
	...
	T.Run("skips a nil row rather than panicking on it", func(t *testing.T) {
		...
		store := &passwordresetmock.StoreMock{
			ListForUserFunc: func(
				...
				return []*passwordreset.Token{nil, tokenIn(scope, "token_1"), nil}, nil
			},
		}
		...
		test.EqOp(t, "token_1", collected[0].ID)
	})

	T.Run("a subject who never asked is a domain holding nothing", func(t *testing.T) {
		t.Parallel()
	...
	T.Run("reports a resolver that could not answer", func(t *testing.T) {
		...
		collector, err := privacy.NewCollector(&passwordresetmock.StoreMock{}, &testReader{}, privacy.RequestScope)
		must.NoError(t, err)
...
func TestNewEraser(T *testing.T) {
	...
	T.Run("refuses what it cannot work without", func(t *testing.T) {
		...
		_, err = privacy.NewEraser(&passwordresetmock.StoreMock{}, nil)
		must.ErrorIs(t, err, privacy.ErrNilScopeResolver)
	})
}
...
func TestEraser_Erase(T *testing.T) {
	...
	T.Run("sums what it destroyed across every scope", func(t *testing.T) {
		...
		store := &passwordresetmock.StoreMock{
			DeleteForUserFunc: func(
				...
				// The executor is the request's, so the tokens and the rest of
	...
	T.Run("erases rather than revoking", func(t *testing.T) {
		...
		// comment. RevokeForUser spares redeemed rows on purpose, so an eraser
		// built on it would leave every reset the subject actually completed.
		store := &passwordresetmock.StoreMock{
			DeleteForUserFunc: func(
		...
		test.SliceLen(t, 1, store.DeleteForUserCalls())
		test.SliceEmpty(t, store.RevokeForUserCalls())
	})

	T.Run("a subject who never asked is not a failure", func(t *testing.T) {
		t.Parallel()
	...
	T.Run("refuses to run outside a transaction", func(t *testing.T) {
		...
		eraser, err := privacy.NewEraser(&passwordresetmock.StoreMock{}, privacy.FixedScopes(firstScope))
		must.NoError(t, err)
	...
	T.Run("reports a resolver that could not answer", func(t *testing.T) {
		...
		eraser, err := privacy.NewEraser(&passwordresetmock.StoreMock{}, privacy.RequestScope)
		must.NoError(t, err)
