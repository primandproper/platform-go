/*
Package migrations supplies the WebAuthn ceremony session table's DDL, rendered
for a dialect and table prefix.
...
repository, nothing to keep in sync as this package evolves:
	...
	ddl, err := migrations.SQL(dialect.Postgres, webauthncredentials.DefaultTablePrefix)
	// ...
	m, err := migrate.New(dialect.Postgres, myMigrations,
		migrate.WithGeneratedMigration(42, "create_webauthn_sessions_table", ddl),
	)
...
// ceremony session table is created by the consumer's own migration run instead
// of being copied into their repository.
