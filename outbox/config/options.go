...
type options struct {
	...
	writer []outbox.WriterOption
	relay  []outbox.RelayOption
}
...
// WithWriterOptions passes opts to NewWriter, which applies them after the options it
// derives from configuration — so a caller can override anything. The other
// constructors ignore them.
func WithWriterOptions(opts ...outbox.WriterOption) Option {
	return func(o *options) { o.writer = append(o.writer, opts...) }
}

// WithRelayOptions passes opts to NewRelay, which applies them after the options it
// derives from configuration — so a caller can override anything. The other
// constructors ignore them.
func WithRelayOptions(opts ...outbox.RelayOption) Option {
	return func(o *options) { o.relay = append(o.relay, opts...) }
}
