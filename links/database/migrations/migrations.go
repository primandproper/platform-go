/*
Package migrations supplies the action link table's DDL, rendered for a dialect
and table prefix.
...
repository, nothing to keep in sync as this package evolves:
	...
	ddl, err := migrations.SQL(dialect.Postgres, linksdatabase.DefaultTablePrefix)
	// ...
	m, err := migrate.New(dialect.Postgres, myMigrations,
		migrate.WithGeneratedMigration(46, "create_action_links_table", ddl),
	)

Statements is the same DDL split into individually executable statements, the
table before its index, for callers running it some other way — a different
...
// table and the index this package creates.
//
// The index is the reason this is not a check on the prefix alone: a prefix
// that is a legal name on its own can still push the index's name past what an
// engine accepts — a failure that would otherwise surface as a migration that
// half ran.
