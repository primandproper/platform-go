...
// design rather than a coincidence. The last two are reported as errors, and the
// providers do tell them apart — a flag nobody created comes back as
// featureflags.ErrFlagNotFound — but this package deliberately does not act on
// the difference. It only ever asks questions whose false answer is inert: a
// grant flag that is false defers to the plan, and a kill flag that is false
// defers to the plan. Neither direction can be flipped by a flag nobody has
// created or a provider nobody can reach.
//
...
// happen. That evaluation is inert here and, since the providers score a missing
// flag as a resolved answer rather than a provider failure, inert for the rest of
// the process's flags too.
