/*
Command queriesgen writes the canonical sqlc input for the work queue schema,
one file per dialect it serves, from workqueue/internal/queries.
...
the statements the queue executes against the schema workqueue/migrations
renders, at build time, with no database running. What the queue executes is the
...
with workqueue/migrations. `make unison` calls it that way before running the
