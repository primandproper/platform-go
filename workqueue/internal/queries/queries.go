...
// statements in a transaction and a batch is a statement per row or per claim.
// The two sets are generated into two queriers because unison will not put a
// query whose shape differs across dialects into one — see renderSplit.
//
...
func Render(d dialect.Dialect) string {
	...
	default:
		panic(fmt.Errorf("work queue queries for dialect %q: %w", d, dialect.ErrUnsupported))
	}
}

// renderPostgres is the Postgres corpus: nine statements, each one round trip.
