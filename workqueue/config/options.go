...
// nothing — including the depth and age gauges, which are the only way to see
// that a queue has stopped draining.
...
// WithQueueOptions passes opts to NewQueue, which applies them after the options
// it derives from configuration — so a caller can override anything. It is how
// a custom key codec reaches a queue built from configuration, since a codec is
