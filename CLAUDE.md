...
**A schema change is a version, never a second schema.** A `migrations` package renders one `ddl.Schema` — the tables as they stand — and today a DDL change edits those `.sql` files, which is honest only because nothing is deployed against any of this schema. The moment a package has deployments to carry forward it states its history as a `ddl.Migrations`: an ordered run of versioned schemas that renders `Statements` and `SQL` over the whole run and splices `Since(N)` for a database already at N. A table change is then a new version, and never an edit to one that has shipped — a consumer at version 3 splices from 4 onwards, so editing 2 is a change that reaches a fresh install and no existing deployment. What a package does not do is grow an `UpgradeStatements`/`UpgradeSQL` pair of its own. `webhooks` had one and it was deleted rather than generalized: a second `ddl.Schema` whose precondition is the opposite of the first's is a migration runner written one package at a time, carrying no version a consumer can record against and nothing to order it against the next one.
