...
A nested package inherits its parent's tier by longest-prefix match, so a new subpackage is classified by construction; an entry is needed only where a directory sits under a path that belongs to primitives-go. Nine do — `authentication/passwordreset` under the engines that hash and issue, `authorization/database` under the policy interface, `search/sync` under the two indexes, and six more — and each says why in the roster.

**A config that builds a store by dispatching on a provider string is stuck on the domain side.** That is where the boundary was actually crossed — not by the nested stores, which are self-contained. The provider string exists because a second implementation exists, so it belongs with the implementation that created the choice: a config that *takes* a store as a parameter crosses nothing, and one that *builds* one names every package it might build from. `authorization`, `authentication/webauthn` and `authentication/oauth2server` are the worked examples, each split into a primitive `config` and a `database/config` beside the store. The primitive halves went to primitives-go and the three `database/config` packages stayed here, each embedding its counterpart's `Config` with no `env` tag on the embed, so a consumer's environment variables resolve unchanged (`caarlos0/env` reads through an anonymous embedded struct and still composes a nested `envPrefix`; YAML needs `yaml:",inline"`, JSON and TOML promote on their own). Each of the six packages' `doc.go` carries the ruling, three on each side of the split.
