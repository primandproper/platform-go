...
`internal/sentinelmatrix` is the roster: every exported `Err` in those eight packages is recorded as `mapped`, `platform` (it wraps a platform sentinel) or `unhandled` (a 500 is the honest answer), checked against the packages' own source in both directions and against what the mappers actually do. A sentinel added later is in none of the three and fails there rather than as `codes.Unknown` in a consumer.
