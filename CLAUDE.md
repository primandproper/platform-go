...
Transport mappings split by tier, and the split is now a module boundary. primitives-go's `errors/http` and `errors/grpc` map the primitives — `database`, `circuitbreaking`, `ratelimiting`, `idempotency`, `requestsigning`, the two search indexes, the `platformerrors` sentinels — and import those packages, so nothing in them may import `errors/http` or `errors/grpc` back. Everything built on top maps itself: `authentication/oauth2clients`, `authentication/signin`, `dataprivacy`, `identity`, `issuereports`, `links`, `operations` and `sessions` each export an `HTTPMapper` and a `GRPCMapper` beside their sentinels and import the two transport packages. A sentinel whose own wording is meant for the person reading it goes to `grpcerrors.RegisterClientSafeSentinels` as well, or gRPC sends the code's name instead.

**The domain tier registers at the composition root, in one call.** `errormappers.Register()` installs all eight pairs plus the `ClientSafeSentinels` of `links`, `identity`, `authentication/signin`, `authentication/oauth2clients` and `issuereports`; `service.Register` calls it for a service built from a `service.Config` and a service assembled by hand calls it itself, next to the mappers it declares for its own sentinels. It lives outside `service` so that a consumer wiring three packages by hand does not import the whole config tree to get it, and it registers unconditionally — an unused mapper is one comparison against a sentinel the process cannot produce, and the expensive direction to be wrong in is a 500 for an error somebody took the trouble to give a status. No `init()` — a mapper that installs itself by being linked in is a side effect a consumer cannot opt out of. `operations/http.New` is the single exception, registering its own HTTP mapper because it is the only surface in the module that both answers through `errors/http` and belongs to a package on that list; `dataprivacy` and `links` ship no transport, `sessions/http` never writes an error response, and `identity/grpc`, `authentication/signin/grpc`, `authentication/oauth2clients/grpc` and `issuereports/grpc` answer through `errors/grpc` rather than `errors/http`. No gRPC surface follows the exception onto its own side: one door stays one door, and each `NewServer`'s documentation names the call a consumer owes instead.
