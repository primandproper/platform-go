...
func TestConfig_EnsureDefaults(t *testing.T) {
	...
	cfg := &Config{}
	cfg.EnsureDefaults()
	test.EqOp(t, settings.DefaultTablePrefix, cfg.TablePrefix)
...
func TestNewStore(T *testing.T) {
	...
	T.Run("refuses a nil client", func(t *testing.T) {
		...
		must.ErrorIs(t, err, settings.ErrNilDatabaseClient)
		test.Nil(t, store)
	})
	...
	T.Run("applies explicit options after the config's", func(t *testing.T) {
		...
		store, err := NewStore(t.Context(), &Config{TablePrefix: "ddb"}, newClient(dialect.Postgres),
			WithStoreOptions(settings.WithTablePrefix("override")))
		must.NoError(t, err)
		must.NotNil(t, store)
	})
