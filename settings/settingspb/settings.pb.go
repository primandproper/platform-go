...
type ListDefinitionsRequest struct {
	...
	// instruction: the rows it reaches are settings retired from the catalog,
	// and the field is honored only for a caller holding
	// settings.definitions.archive and cleared for everybody else. See the file
	// comment.
...
type ListValuesForDefinitionRequest struct {
	...
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// filter pages the result. Its include_archived is a request rather than an
	// instruction: the rows it reaches are answers subjects have since cleared,
	// and the field is honored only for a caller holding settings.values.write
	// and cleared for everybody else. See the file comment.
