...
func TestNewOptions(T *testing.T) {
	...
	T.Run("skips nil options", func(t *testing.T) {
		...
		o := newOptions([]Option{
			nil,
			WithDatabaseOptions(rbac.WithLogger(loggingnoop.NewLogger())),
			nil,
		})
...
func TestWithDatabaseOptions(T *testing.T) {
	...
	T.Run("accumulates across calls", func(t *testing.T) {
		...
		o := newOptions([]Option{
			WithDatabaseOptions(rbac.WithLogger(loggingnoop.NewLogger())),
			WithDatabaseOptions(
				rbac.WithTracerProvider(tracingnoop.NewTracerProvider()),
				rbac.WithMetricsProvider(metricsnoop.NewMetricsProvider()),
			),
		})
