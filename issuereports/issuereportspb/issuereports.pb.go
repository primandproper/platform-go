...
//
// A second hazard rides with read-modify-write and is a different one, because
// somebody guarded against the first may believe they are guarded against this.
// There is no version on a revision, so two operators who read the same report
// and each change one field lose whichever writes first — the second sends the
// values it read, which are stale for the field the first changed. That is a
// lost update rather than a blanked field, and merge semantics would not fix
// it: a merge of stale values is still stale. TransitionReport takes an
// expected status for exactly this reason and UpdateReport takes nothing; a
// deployment where concurrent revision is real wants a guard of that shape,
// which is additive whenever somebody needs it.
//
...
type ListReportsRequest struct {
	...
	// instruction: the rows it reaches are reports somebody took out of the
	// queue, and the field is honored only for a caller holding
	// issues.reports.archive and cleared for everybody else. See the file
	// comment.
