/*
Package migrations supplies the issue reports table's DDL, rendered for a
...
repository, nothing to keep in sync as this package evolves:
	...
	ddl, err := migrations.SQL(dialect.Postgres, issuereports.DefaultTablePrefix)
	// ...
	m, err := migrate.New(dialect.Postgres, myMigrations,
	    migrate.WithGeneratedMigration(61, "create_issue_reports_table", ddl),
	)
...
// and sorted.
//
// It is the complete list — issuereports creates no table this omits, because
