...
func TestNewRefusesAnEmptyTarget(T *testing.T) {
	...
	test.ErrorIs(T, err, issuereportsclient.ErrEmptyTarget)
	test.True(T, platformerrors.Is(err, platformerrors.ErrEmptyInputParameter))
}

// TestNewSuppliesNoTransportSecurity is worth pinning on a service whose pages
// are what your users said about things that happened to them. A default of
// insecure credentials would be this package deciding to put that on the wire in
// the clear, so grpc.NewClient refusing a target with no credentials option is
// the right failure and is left in place.
func TestNewSuppliesNoTransportSecurity(T *testing.T) {
	T.Parallel()

	c, err := issuereportsclient.New("passthrough:///nowhere")
	if c != nil {
...
func TestNewDialsWithCredentials(T *testing.T) {
	...
	c, err := issuereportsclient.New("passthrough:///nowhere",
		issuereportsclient.WithDialOptions(grpc.WithTransportCredentials(insecure.NewCredentials())))
	must.NoError(T, err)
	must.NotNil(T, c)

	// Every RPC is a method on the result, by embedding, which is the whole
	// reason this package adds none of its own — ten wrappers would be ten
	// things that can drift from the schema.
	var _ issuereportspb.IssueReportsServiceClient = c
...
func TestCloseOnAWrappedClientClosesNothing(T *testing.T) {
	...
	c := issuereportsclient.Wrap(conn)
	must.NotNil(T, c)
...
func TestDefaultInterceptorsIsADialOption(T *testing.T) {
	...
	test.NotNil(T, issuereportsclient.DefaultInterceptors())
}
...
// errors.Is against an issuereports sentinel that never matches; what is checked here
// is that declining them is not declining the client.
func TestWithoutDefaultInterceptorsBuildsAClient(T *testing.T) {
	T.Parallel()

	c, err := issuereportsclient.New("passthrough:///nowhere",
		issuereportsclient.WithoutDefaultInterceptors(),
		issuereportsclient.WithDialOptions(grpc.WithTransportCredentials(insecure.NewCredentials())))
	must.NoError(T, err)
	must.NotNil(T, c)
