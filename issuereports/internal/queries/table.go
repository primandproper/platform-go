...
// Table is the issue reports table's shape.
//
...
//
// It is deliberately the same shape notifications/internal/queries declares.
// This table is keyed on more than the scope for its writes and on the scope
// alone for its reads, so it takes no standard set and there is no Omitted to
// declare; what a statement keys on is written at the statement instead, which
// is what querygen.Match is for.
type Table struct {
...
// a DEFAULT — see issuereports/migrations. A caller-supplied creation time is
