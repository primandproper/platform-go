...
var rulings = map[string]ruling{
	...
	"authentication/oauth2serverstore/internal/oauth2serverdb":    {tier: unison},
	...
	"shredding/internal/shreddingdb":                              {tier: unison},
	"authentication/webauthnsessions/internal/webauthnsessionsdb": {tier: unison},
	"audit/internal/auditdb":                                      {tier: unison},
	"rbac/internal/authorizationdb":                               {tier: unison},
	"settings/internal/settingsdb":                                {tier: unison},
	"mediaregistry/internal/registrydb":                           {tier: unison},
	"notifications/internal/notificationsdb":                      {tier: unison},
	"operations/internal/operationsdb":                            {tier: unison},
	"operations/internal/operationssplitdb":                       {tier: unison},
	"timers/internal/timersdb":                                    {tier: unison},
	"timers/internal/timerssplitdb":                               {tier: unison},
	...
	"operations/internal/queries": {tier: exempt, why: "a corpus source, like the generator it renders through: its literals are the statements the committed .sql is rendered from, checked by sqlc and executed through operations/internal/operationsdb or operations/internal/operationssplitdb"},
	...
	"timers/internal/queries":     {tier: exempt, why: "a corpus source whose two statement sets are written out in full rather than rendered from database/querygen, for the reason its own doc gives: every one of them assigns an expression, and each is still checked by sqlc and executed through timers/internal/timersdb or timers/internal/timerssplitdb"},
	...
	"rbac":                                {tier: none, why: "the resolver whose thirteen fmt.Sprintf builders a survey counted as zero: its statements are rendered by rbac/internal/queries and executed through the querier above, so the package that used to compose them holds none"},
	...
	"authentication/signin/refreshtokens": {tier: none, why: "born on the tier: the six statements its store executes are rendered by authentication/signin/refreshtokens/internal/queries and reached through the querier above, so the package that would have composed them never did"},
	"authentication/signin/magiclinks":    {tier: none, why: "born on the tier: the five statements its store executes are rendered by authentication/signin/magiclinks/internal/queries and reached through the querier above, so the package that would have composed them never did"},
	"authentication/signin/recoverycodes": {tier: none, why: "born on the tier: the six statements its store executes are rendered by authentication/signin/recoverycodes/internal/queries and reached through the querier above, so the package that would have composed them never did"},
