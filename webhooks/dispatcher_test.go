...
type fakeStore struct {
	...
	markDelivered       func(ctx context.Context, dispatchID, claimedBy string, at time.Time) error
	recordFailure       func(ctx context.Context, dispatchID, claimedBy string, attempts int, nextAttempt time.Time, lastErr string, dead bool) error
...
func (f *fakeStore) MarkDelivered(ctx context.Context, dispatchID, claimedBy string, at time.Time) error {
	if f.markDelivered == nil {
		return nil
	}

	return f.markDelivered(ctx, dispatchID, claimedBy, at)
}

func (f *fakeStore) RecordFailure(ctx context.Context, dispatchID, claimedBy string, attempts int, nextAttempt time.Time, lastErr string, dead bool) error {
	if f.recordFailure == nil {
		return nil
	}

	return f.recordFailure(ctx, dispatchID, claimedBy, attempts, nextAttempt, lastErr, dead)
}
