...
CREATE TABLE IF NOT EXISTS {{PREFIX}}webhooks_endpoints (
    ...
    KEY {{PREFIX}}webhooks_endpoints_live_idx (archived_at, created_at, id),

    -- Serves the registry read, which is always one scope's page ordered by id,
    -- and is available to the fan-out join's scope predicate. Leading with
    -- scope is what keeps one tenant's page from walking every other tenant's
    -- rows.
    KEY {{PREFIX}}webhooks_endpoints_scope_idx (scope, archived_at, id)
);
...
CREATE TABLE IF NOT EXISTS {{PREFIX}}webhooks_subscriptions (
    ...
    KEY {{PREFIX}}webhooks_subscriptions_event_idx (event_type, endpoint_id),

    -- Serves the paged read of one endpoint's subscriptions, which is
    -- cursor-ordered on id. MySQL has no partial indexes, so archived_at is a
    -- leading column here rather than a WHERE clause, for the same reason the
    -- dispatch indexes below carry delivered_at.
    KEY {{PREFIX}}webhooks_subscriptions_endpoint_idx (endpoint_id, archived_at, id),
...
CREATE TABLE IF NOT EXISTS {{PREFIX}}webhooks_dispatches (
    ...
    -- the whole table and the predicate columns lead. The claim and ordering
    -- queries filter on delivered_at/dead first, so putting them in front keeps
    -- the index selective for the same queries the partial clause serves
    -- elsewhere.
    KEY {{PREFIX}}webhooks_dispatches_claim_idx
    ...
    -- never another subscriber's. id is in the key because the predicate orders
    -- on (created_at, id) — dispatches fanned out from one delivery share a
    -- timestamp and are separated only by id.
    KEY {{PREFIX}}webhooks_dispatches_ordering_idx
        (endpoint_id, ordering_key, delivered_at, dead, created_at, id),

    -- Serves the reaper.
    KEY {{PREFIX}}webhooks_dispatches_reap_idx (delivered_at, id),
...
CREATE TABLE IF NOT EXISTS {{PREFIX}}webhooks_attempts (
    ...
    KEY {{PREFIX}}webhooks_attempts_delivery_idx (delivery_id, created_at, id),
    KEY {{PREFIX}}webhooks_attempts_endpoint_idx (endpoint_id, created_at, id)
);
