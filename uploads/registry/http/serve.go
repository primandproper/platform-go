...
const (
	objectIDKey = registry.ObjectAttributeKey
...
func New(
	...
	switch {
	case store == nil:
		return nil, registry.ErrNilStore
	case client == nil:
		return nil, registry.ErrNilDatabaseClient
	case manager == nil:
		return nil, registry.ErrNilUploadManager
	}
...
// are entitled to it.
//
// Every refusal it makes is registry.ErrObjectNotFound, whatever produced it. A
...
func (h *Handler) read(ctx context.Context, span observability.Operation, objectID string) (*registry.Object, error) {
	if objectID == "" {
		return nil, platformerrors.Wrap(registry.ErrObjectNotFound, "no object named")
	}
	...
	if object == nil {
		return nil, platformerrors.Wrapf(registry.ErrObjectNotFound, "object %q", objectID)
	}
...
func modTimeOf(object *registry.Object) time.Time {
