...
// WithTablePrefix namespaces the two notifications tables. It must match the
...
// for tests that need a registration and a read to be a known distance apart. A
// nil clock is ignored.
//
// It does not reach created_at or last_updated_at, which the database assigns
// from its own clock — see notifications/migrations. What it reaches is the two
// stamps a caller supplies: when a notification was read, and when a device last
// announced itself.
