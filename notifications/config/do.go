...
// keys: notifications.Store, which is what [NewStore] builds, and the three
// narrower seams the same value satisfies — notifications.Inbox,
// notifications.Registry and mobile.TokenInvalidator.
//
// The four are one store rather than four. The three are registered as
// narrowings of the notifications.Store registration, so a container that
// invokes the inbox and the registry gets the same value, holding one
// connection and one set of instruments.
//
...
func RegisterStore(i do.Injector) {
	...
	// to be nil: a resolution that failed answers with a nil Store, and handing
	// that straight back would register a seam whose error is gone and whose
	// value is not there.
