...
type Config struct {
	...
	// OAuth2Clients registers both the client registry's store and its service,
	// which is everything its gRPC surface mounts over. A container that also
	// registers an oauth2clients.Store or an *oauth2clients.Service by hand —
	// which was the only way to mount the surface before this field existed —
	// holds two providers under one key, and samber/do panics on the second
	// registration. Configure the block or keep the hand registration, not both.
