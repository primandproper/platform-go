...
func userFromRow(r *identitydb.GetUserRow) *User {
	return &User{
		...
		Scope:                                  r.Scope,
		Username:                               r.Username,
		DisplayName:                            displayedName(r.Username, r.DisplayName),
		...
		EmailAddressVerificationTokenDigest:    r.EmailAddressVerificationTokenDigest,
		EmailAddressVerificationTokenExpiresAt: r.EmailAddressVerificationTokenExpiresAt,
...
func userFromEmailVerificationTokenDigestRow(r *identitydb.GetUserByEmailVerificationTokenDigestRow) *User {
	return userFromRow(&identitydb.GetUserRow{
		ID:                                     r.ID,
		Scope:                                  r.Scope,
		Username:                               r.Username,
...
func createUserParams(u *User) identitydb.CreateUserParams {
	return identitydb.CreateUserParams{
		...
		EmailAddressVerificationTokenDigest:    tokenDigest(u.EmailAddressVerificationToken),
		EmailAddressVerificationTokenExpiresAt: u.EmailAddressVerificationTokenExpiresAt,
...
// updateUserParams is the profile update: five columns and three derived ones.
//
// verifiedAt, digest and expiresAt are the caller's decision rather than the
// user's own fields, and they are decided together — moving an address clears
// the proof that went with it and burns the link that would have proved it,
// deadline and all, and only the store knows what the stored address was. Taking the digest from the User would
// be worse than useless here: the copy a caller holds is usually a redacted one,
// whose digest is the empty string, so every profile save would burn a live
// link. See SQLStore.UpdateUser.
//
// It is a digest rather than a secret, because it is carried forward off the
// stored row rather than minted: this statement either leaves the column as it
// found it or clears it, and neither needs the token the digest is of.
func updateUserParams(
	u *User,
	verifiedAt *time.Time,
	digest string,
	expiresAt *time.Time,
) identitydb.UpdateUserParams {
	return identitydb.UpdateUserParams{
		...
		EmailAddressVerificationTokenDigest:    digest,
		EmailAddressVerificationTokenExpiresAt: expiresAt,
	}
}
...
func memberPageRow(r *identitydb.ListAccountMembersRow) pageRow[MembershipWithUser] {
	user := userFromRow(&identitydb.GetUserRow{
		...
		EmailAddressVerificationTokenDigest:    r.UserEmailAddressVerificationTokenDigest,
		EmailAddressVerificationTokenExpiresAt: r.UserEmailAddressVerificationTokenExpiresAt,
...
func createInvitationParams(i *Invitation) identitydb.CreateInvitationParams {
	return identitydb.CreateInvitationParams{
		...
		ToName:           i.ToName,
		ToUser:           i.ToUser,
		TokenDigest:      tokenDigest(i.Token),
