...
const getInvitationSQLite = `SELECT
	...
	{{prefix}}identity_invitations.token_digest,
...
const getUserByEmailVerificationTokenDigestSQLite = `SELECT
	{{prefix}}identity_users.id,
	{{prefix}}identity_users.scope,
	{{prefix}}identity_users.username,
	{{prefix}}identity_users.display_name,
...
const markUserEmailAddressProvenSQLite = `UPDATE {{prefix}}identity_users SET
...
const markUsersAsIndexedSQLite = `UPDATE {{prefix}}identity_users SET
...
const scanUserIDsForReindexSQLite = `SELECT {{prefix}}identity_users.id
FROM {{prefix}}identity_users
WHERE {{prefix}}identity_users.archived_at IS NULL
	AND {{prefix}}identity_users.id COLLATE BINARY > ?1
ORDER BY {{prefix}}identity_users.id COLLATE BINARY
LIMIT COALESCE(?2, 50)`
...
type sqliteQueries struct {
	...
	getUserByEmailVerificationTokenDigest    string
...
func newSQLite(prefix string) *sqliteQueries {
	return &sqliteQueries{
		...
		getUserByEmailVerificationTokenDigest:    strings.ReplaceAll(getUserByEmailVerificationTokenDigestSQLite, prefixMarker, prefix),
		...
		markUserEmailAddressProven:               strings.ReplaceAll(markUserEmailAddressProvenSQLite, prefixMarker, prefix),
		...
		markUsersAsIndexed:                       strings.ReplaceAll(markUsersAsIndexedSQLite, prefixMarker, prefix),
		...
		scanUserIDsForReindex:                    strings.ReplaceAll(scanUserIDsForReindexSQLite, prefixMarker, prefix),
...
// GetUserByEmailVerificationTokenDigest runs the :one query against sqlite.
func (q *sqliteQueries) GetUserByEmailVerificationTokenDigest(ctx context.Context, db DBTX, arg GetUserByEmailVerificationTokenDigestParams) (GetUserByEmailVerificationTokenDigestRow, error) {
	row := db.QueryRowContext(ctx, q.getUserByEmailVerificationTokenDigest,
		arg.EmailAddressVerificationTokenDigest,
		arg.Scope,
	)

	var i GetUserByEmailVerificationTokenDigestRow
...
// MarkUserEmailAddressProven runs the :execrows query against sqlite.
func (q *sqliteQueries) MarkUserEmailAddressProven(ctx context.Context, db DBTX, arg MarkUserEmailAddressProvenParams) (int64, error) {
...
// MarkUsersAsIndexed runs the :execrows query against sqlite.
func (q *sqliteQueries) MarkUsersAsIndexed(ctx context.Context, db DBTX, arg MarkUsersAsIndexedParams) (int64, error) {
...
// ScanUserIDsForReindex runs the :many query against sqlite.
func (q *sqliteQueries) ScanUserIDsForReindex(ctx context.Context, db DBTX, arg ScanUserIDsForReindexParams) ([]ScanUserIDsForReindexRow, error) {
