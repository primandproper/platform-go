...
SELECT
	...
	(
		...
		WHERE identity_memberships.created_at > COALESCE(sqlc.narg(created_after), (SELECT CURRENT_TIMESTAMP - INTERVAL 999 YEAR))
			AND identity_memberships.created_at < COALESCE(sqlc.narg(created_before), (SELECT CURRENT_TIMESTAMP + INTERVAL 999 YEAR))
			AND (
				identity_memberships.last_updated_at IS NULL
				OR identity_memberships.last_updated_at > COALESCE(sqlc.narg(updated_after), (SELECT CURRENT_TIMESTAMP - INTERVAL 999 YEAR))
			)
			AND (
				identity_memberships.last_updated_at IS NULL
				OR identity_memberships.last_updated_at < COALESCE(sqlc.narg(updated_before), (SELECT CURRENT_TIMESTAMP + INTERVAL 999 YEAR))
			)
