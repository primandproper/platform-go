...
func TestNewRefusesAnEmptyTarget(T *testing.T) {
	...
	test.ErrorIs(T, err, commentsclient.ErrEmptyTarget)
	test.True(T, platformerrors.Is(err, platformerrors.ErrEmptyInputParameter))
}

// TestNewSuppliesNoTransportSecurity: what travels over this connection is what
// people said to each other, so a default of insecure credentials would be this
// package putting that on the wire in the clear on somebody's behalf.
...
func TestNewSuppliesNoTransportSecurity(T *testing.T) {
	...
	c, err := commentsclient.New("passthrough:///nowhere")
	if c != nil {
...
func TestNewDialsWithCredentials(T *testing.T) {
	...
	c, err := commentsclient.New("passthrough:///nowhere",
		commentsclient.WithDialOptions(grpc.WithTransportCredentials(insecure.NewCredentials())))
	must.NoError(T, err)
	must.NotNil(T, c)

	// Every RPC is a method on the result, by embedding, which is the whole
	// reason this package adds none of its own.
	var _ commentspb.CommentsServiceClient = c
...
func TestDefaultInterceptorsIsADialOption(T *testing.T) {
	...
	test.NotNil(T, commentsclient.DefaultInterceptors())
}
...
// errors.Is against a comments sentinel that never matches; what is checked here
// is that declining them is not declining the client.
func TestWithoutDefaultInterceptorsBuildsAClient(T *testing.T) {
	T.Parallel()

	c, err := commentsclient.New("passthrough:///nowhere",
		commentsclient.WithoutDefaultInterceptors(),
		commentsclient.WithDialOptions(grpc.WithTransportCredentials(insecure.NewCredentials())))
	must.NoError(T, err)
	must.NotNil(T, c)
