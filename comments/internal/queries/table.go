...
// Table is the comments table's shape.
//
...
//
// It is deliberately the same shape issuereports/internal/queries declares. This
// table is keyed on more than the scope for its writes and on the scope alone
// for its reads, so it takes no standard set and there is no Omitted to declare;
...
// a DEFAULT — see comments/migrations. A caller-supplied creation time is how a
// row ends up with one that disagrees with its id, and the cursor walk orders by
// id while the filter window compares created_at.
