/*
Package mediaregistrycfg assembles an upload registry Store from environment
configuration.
...
type Config struct {
	...
	// migrations were rendered with. Defaults to mediaregistry.DefaultTablePrefix.
...
func (cfg *Config) EnsureDefaults() {
	if cfg.TablePrefix == "" {
		cfg.TablePrefix = mediaregistry.DefaultTablePrefix
	}
}
...
func NewStore(ctx context.Context, cfg *Config, client database.Client, opts ...Option) (mediaregistry.Store, error) {
	...
	base := []mediaregistry.SQLStoreOption{
		mediaregistry.WithTablePrefix(cfg.TablePrefix),
		mediaregistry.WithStoreLogger(options.logger),
		mediaregistry.WithStoreTracerProvider(options.tracerProvider),
