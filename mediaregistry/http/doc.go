/*
Package http serves the bytes behind a mediaregistry row, to the callers the row
entitles.

	handler, err := mediaregistryhttp.New(store, client, manager,
		mediaregistryhttp.WithCallerResolver(callerFromSession),
...
mediaregistry's documentation heads a section "Why the row is the access
...
The failure it prevents is the one mediaregistry names. A consumer without
this serves objects public-by-key, which makes an unguessable key the only protection
...
uploads is a primitive: it left in the v14 split, and the parent directory it
had left behind here went with the flattening, so the row store is
mediaregistry at the root. A domain package importing a primitive is the
direction the split bought, so this package reaching for uploads.UploadManager
inverts nothing. What
...
another tenant reads as absent because that is what mediaregistry's store
...
mediaregistry.Store.GetObjectByKey therefore goes unused here. A consumer that has
...
The nine store methods stay off the wire. There is no metadata surface here —
