/*
Package migrations supplies the waitlist tables' DDL, rendered for a dialect and
table prefix.
...
repository, nothing to keep in sync as this package evolves:
	...
	ddl, err := migrations.SQL(dialect.Postgres, waitlists.DefaultTablePrefix)
	// ...
	m, err := migrate.New(dialect.Postgres, myMigrations,
		migrate.WithGeneratedMigration(52, "create_waitlist_tables", ddl),
	)
...
is in that list the moment it is added.
	...
	names, err := migrations.Tables(waitlists.DefaultTablePrefix)
...
audit. The alternative is copying two names out of the .sql files, which is a
...
the row's id disagrees with the order a cursor walk pages in. The waitlist store
...
# closes_at is NOT NULL, and contact_digest has no default

Those are the two columns that carry a decision rather than a convention, and
both are written out in the Postgres schema beside the columns themselves.

closes_at is NOT NULL because a nullable one turns the read this package exists
to serve — the lists still taking signups — into a disjunction over the column
it pages by, on three dialects. A list whose end is not yet known names a far
horizon; a list that should stop now is archived.

contact_digest has no default because a signup row without one is a signup
nothing can ever suppress, and suppression is the obligation the pair of contact
columns exists for. contact does default to the empty string, which is what a
withdrawal leaves behind.

# The signups reference cascades

waitlist_signups references the list it hangs off with ON DELETE CASCADE, and
the cascade is the point rather than a default. A list is only ever hard-deleted
deliberately — archiving it is the ordinary way to retire one — and a deletion
that left signups behind would leave rows referring to a list that no longer
says when it closed or what it was for.
...
// prefix and sorted.
//
// It is the complete list — waitlists creates no table this omits, because the
...
//
// It is not an ordering a caller can delete in. The signups table references the
// lists table, so a consumer clearing these tables wants the dialect's own way
// of ignoring the constraints rather than a sequence read off this slice.
