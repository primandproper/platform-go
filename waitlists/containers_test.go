...
// Postgres and SQLite have and MySQL does not, and — the one this catches — the
// closing time compared against a real temporal type rather than against text.
//
// SQLite stores a DATETIME as text and compares it lexicographically, which
// works for the UTC shape this store writes and tells us nothing about whether
// the bound horizon in ListOpenLists is being read as a timestamp on a server
// that has one. The same goes for the guarded updates' affected-row counts,
// which are what the whole lifecycle rests on and which each driver reports its
// own way.
...
// spans the scope, the list reference and the contact digest, and InnoDB bounds
// a key at 3072 bytes — so a column widened without counting them is a CREATE
// that fails on the server and nowhere else.
