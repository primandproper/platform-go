...
type ListListsRequest struct {
	...
	// instruction: the rows it reaches are lists withdrawn from the catalog, and
	// the field is honored only for a caller holding waitlists.lists.archive
...
type ListSignupsRequest struct {
	...
	ListId string                 `protobuf:"bytes,1,opt,name=list_id,json=listID,proto3" json:"list_id,omitempty"`
	// filter pages the result. Its include_archived is a request rather than an
	// instruction: the rows it reaches are signups retired administratively, and
	// the field is honored only for a caller holding waitlists.signups.archive
	// and cleared for everybody else. See the file comment.
