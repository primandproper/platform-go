...
func TestNewRefusesAnEmptyTarget(T *testing.T) {
	...
	test.ErrorIs(T, err, billingclient.ErrEmptyTarget)
...
// are what an account has paid. A default of insecure credentials would be this
// package deciding to put a customer's ledger on the wire in the clear, so
// grpc.NewClient refusing a target with no credentials option is the right
// failure and is left in place.
func TestNewSuppliesNoTransportSecurity(T *testing.T) {
	T.Parallel()

	c, err := billingclient.New("passthrough:///nowhere")
	if c != nil {
...
func TestNewDialsWithCredentials(T *testing.T) {
	...
	c, err := billingclient.New("passthrough:///nowhere",
		billingclient.WithDialOptions(grpc.WithTransportCredentials(insecure.NewCredentials())))
	must.NoError(T, err)
	must.NotNil(T, c)

	// Every RPC is a method on the result, by embedding, which is the whole
	// reason this package adds none of its own — eighteen wrappers would be
	// eighteen things that can drift from the schema.
	var _ billingpb.BillingServiceClient = c
...
func TestDefaultInterceptorsIsADialOption(T *testing.T) {
	...
	test.NotNil(T, billingclient.DefaultInterceptors())
}
...
// errors.Is against a billing sentinel that never matches; what is checked here
// is that declining them is not declining the client.
func TestWithoutDefaultInterceptorsBuildsAClient(T *testing.T) {
	T.Parallel()

	c, err := billingclient.New("passthrough:///nowhere",
		billingclient.WithoutDefaultInterceptors(),
		billingclient.WithDialOptions(grpc.WithTransportCredentials(insecure.NewCredentials())))
	must.NoError(T, err)
	must.NotNil(T, c)
